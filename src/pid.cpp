...
void moveDistance(
    double distanceInFeet) { // Sychronous function. Will block main thread.
  ...
  double dTarget =
      (((double)(leftQuad.get_value() + rightQuad.get_value())) / 2) +
      targetTickTravel; // Signifies the average tick position given the
                        // distance needed
  ...
  double sTarget = imu.get_heading(); // Signifies the difference between the
                                      // encoders at the start
  double sError = 0;
  ...
  while ((dError >= 10 || sError >= 0.5) &&
         currentTime <=
             (375 * std::abs(distanceInFeet))) { // 2 PID Loop with Threshold
    ...
    sErrorSigned = ((std::abs(sErrorSigned - sLastError) <
                     std::abs(sErrorSigned - sLastError + 360)) &&
                            (std::abs(sErrorSigned - sLastError) <
                             std::abs(sErrorSigned - sLastError - 360))
                        ? sErrorSigned
                        : ((std::abs(sErrorSigned - sLastError + 360) <
                            std::abs(sErrorSigned - sLastError)) &&
    ...
    double dd = (dError - dLastError) *
                dkd; // Derivative times derivative gain constant
    dLastError = dError;
    ...
    double sd = (sError - sLastError) *
                skd; // Derivative times derivative gain constant
    sLastError = sError;
    ...
    if (dDirection * sDirection == 1) { // Left Side is too far forward. spid
                                        // will reduce motor speed on left side.
      rightSideCorrection = spid;
    } else if (dDirection * sDirection == -1) {
      leftSideCorrection = spid; // Right Side is too far forward. spid will
                                 // reduce motor speed on right side.
    }
...
void turnAngle(
    double angleInDegrees) { // Sychronous function. Will block main thread.
  // Constants
  double kp = 2.7;
  double ki = 0;
  double kd = 3;
  ...
  while (error >= 0.5 &&
         currentTime <=
             (std::abs(angleInDegrees) * 9.5) +
                 350) { // PID loop with threshold //Add timeout if needed!!!
    pros::lcd::set_text(2, std::to_string(imu.get_heading()));
    pros::lcd::set_text(3, std::to_string(leftQuad.get_value()));
    pros::lcd::set_text(4, std::to_string(rightQuad.get_value()));
    ...
    signedError =
        ((std::abs(signedError - lastSignedError) <
          std::abs(signedError - lastSignedError + 360)) &&
                 (std::abs(signedError - lastSignedError) <
                  std::abs(signedError - lastSignedError - 360))
             ? signedError
             : ((std::abs(signedError - lastSignedError + 360) <
    ...
    double d =
        (error - lastError) * kd; // Derivative times derivative gain constant
    lastError = error;
    ...
    rightWheel1.move(-power *
                     direction); // Running in rpm values from -200 to 200.
